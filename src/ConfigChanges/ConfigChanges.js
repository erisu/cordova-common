...
function process (plugins_dir, project_dir, platform, platformJson, pluginInfoProvider) {
...
class PlatformMunger {
    constructor (platform, project_dir, platformJson, pluginInfoProvider, configKeeper) {
        this.project_dir = project_dir;
        this.config_keeper = configKeeper || new ConfigKeeper(project_dir);
    ...
    apply_file_munge (file, munge, remove) {
        for (let selector in munge.parents) {
            for (let xml_child in munge.parents[selector]) {
                // this xml child is new, graft it (only if config file exists)
                let config_file = this.config_keeper.get(this.project_dir, this.platform, file);

                if (config_file.exists && remove) {
                    config_file.prune_child(selector, munge.parents[selector][xml_child]);
                } else if (config_file.exists) {
                    config_file.graft_child(selector, munge.parents[selector][xml_child]);
                } else {
    ...
    remove_plugin_changes (pluginInfo, is_top_level) {
        let platform_config = this.platformJson.root;
        let plugin_vars = is_top_level ?
        ...
        let config_munge = this.generate_plugin_config_munge(pluginInfo, plugin_vars, edit_config_changes);
        // global munge looks at all plugins' changes to config files
        let global_munge = platform_config.config_munge;
        let munge = mungeutil.decrement_munge(global_munge, config_munge);

        for (let file in munge.files) {
    ...
    add_plugin_changes (pluginInfo, plugin_vars, is_top_level, should_increment, plugin_force) {
        ...
        if (!edit_config_changes || edit_config_changes.length === 0) {
            ...
            let isConflictingInfo = is_conflicting(edit_config_changes, platform_config.config_munge, self, plugin_force);
            ...
            if (plugin_force) {
                ...
                let conflict_munge = mungeutil.decrement_munge(platform_config.config_munge, isConflictingInfo.conflictingMunge);
                for (let conflict_file in conflict_munge.files) {
        ...
        munge_helper(should_increment, this, platform_config, config_munge);
    ...
    add_config_changes (config, should_increment) {
        ...
        if (config.getEditConfigs) {
            let edit_config_changes = config.getEditConfigs(this.platform);
        ...
        if (config.getConfigFiles) {
            let config_files_changes = config.getConfigFiles(this.platform);
        ...
        if (changes && changes.length > 0) {
            let isConflictingInfo = is_conflicting(changes, platform_config.config_munge, this, true /* always force overwrite other edit-config */);
    ...
    reapply_global_munge () {
        ...
        for (let file in global_munge.files) {
    ...
    generate_config_xml_munge (config, config_changes, type) {
        ...
        let id = type === 'config.xml' ? type : config.id;
     ...
     * @param {*} pluginInfo
     * @param {*} vars
     * @param {*} edit_config_changes
     */
    generate_plugin_config_munge (pluginInfo, vars, edit_config_changes) {
        vars = vars || {};

        let munge = { files: {} };
        let changes = pluginInfo.getConfigFiles(this.platform);
    ...
    /**
     * Go over the prepare queue and apply the config munges for each
     * plugin that has been (un)installed.
     *
     * @param {*} plugins_dir
     */
    process (plugins_dir) {
        ...
        platform_config.prepare_queue.uninstalled.forEach((u) => {
            let pluginInfo = this.pluginInfoProvider.get(path.join(plugins_dir, u.plugin));
...
function munge_helper (should_increment, platformMunger, platform_config, config_munge) {
    ...
    for (let file in munge.files) {
        platformMunger.apply_file_munge(file, munge.files[file]);
    }

    return platformMunger;
}

function is_conflicting (editchanges, config_munge, self, force) {
    let files = config_munge.files;
    let conflictFound = false;
    let conflictWithConfigxml = false;
    let conflictingMunge = { files: {} };
    let configxmlMunge = { files: {} };
    let conflictingParent;
    let conflictingPlugin;

    editchanges.forEach(function (editchange) {
        if (files[editchange.file]) {
            let parents = files[editchange.file].parents;
            let target = parents[editchange.target];

            // Check if the edit target will resolve to an existing target
            if (!target || target.length === 0) {
                let file_xml = self.config_keeper.get(self.project_dir, self.platform, editchange.file).data;
                let resolveEditTarget = xml_helpers.resolveParent(file_xml, editchange.target);
                let resolveTarget;
