...
const modules = {};
const addProperty = require('../util/addProperty');
...
function getIOSProjectname (project_dir) {
    ...
    if (matches.length === 1) {
        ...
        const msg = matches.length === 0
            ? 'Does not appear to be an xcode project, no xcode project file in ' + project_dir
            : 'There are multiple *.xcodeproj dirs in ' + project_dir;
