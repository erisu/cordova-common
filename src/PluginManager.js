...
const { PlatformMunger } = require('./ConfigChanges/ConfigChanges');
...
class PluginManager {
     ...
     * @param {Object} project The project to add/remove plugin changes to/from
     */
    constructor (platform, locations, project) {
    ...
    doOperation (operation, plugin, options = {}) {
        if (![PluginManager.INSTALL, PluginManager.UNINSTALL].includes(operation)) return Promise.reject(new CordovaError('The parameter is incorrect. The opeation must be either "add" or "remove"'));
        ...
        plugin.getFilesAndFrameworks(this.platform, options)
            ...
            .forEach(item => {
                ...
                const action = operation === PluginManager.INSTALL ? installer : uninstaller;

                actions.push(actions.createAction(action, actionArgs, uninstaller, actionArgs));
            });

        // ... and run through the action stack
        return actions.process(this.platform)
            .then(() => {
                if (this.project.write) this.project.write();
 ...
 */
PluginManager.get = (platform, locations, ideProject) => new PluginManager(platform, locations, ideProject);
